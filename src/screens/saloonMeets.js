...
const SaloonMeets = () => {
  ...
  useEffect(() => {
    const meetsListele = async () => {
      ...
      const data = await firebase.firestore().collection("meets").where("saloonEmail", "==", userMail).get()
  ...
  const flatData = ({ item }) => {
    return (
      <View style={styles.scrollStyle} >
        ...
        <Text style={styles.scrollTextFont} > {item.randevuAlanName} </Text>
        {/* <Text > {item.phone} </Text> */}
        <Text> {item.clock} </Text>
        <Text > {item.date} </Text>
        </View>
        <TouchableOpacity style={{marginRight:25}} onPress={()=>Linking.openURL(`tel:${item.phone}`)}>
          <AntDesign name='phone' style={{fontSize:32 }}/>
        </TouchableOpacity>
      </View>
    )
  };
